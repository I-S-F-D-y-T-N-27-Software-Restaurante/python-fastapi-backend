...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None):
    to_encode = data.copy()
    expire = datetime.now(timezone.utc) + (expires_delta if expires_delta else timedelta(minutes=float(os.getenv('ACCESS_TOKEN_EXPIRE_MINUTES', '30'))))
    to_encode.update({"exp": expire})
    return jwt.encode(to_encode, SECRET_KEY, algorithm=os.getenv('ALGORITHM', 'HS256'))

def verify_jwt_token(token: str) -> dict:
    try:
        return jwt.decode(token, SECRET_KEY, algorithms=[os.getenv('ALGORITHM', 'HS256')])
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if scheme.lower() != "bearer":
                return JSONResponse(status_code=401, content={"detail": "Esquema inválido. Use 'Bearer <token>'"})
...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    return {"user_id": user_id, "sub": email, "roles": payload.get("roles", []), "permissions": payload.get("permissions", [])}
